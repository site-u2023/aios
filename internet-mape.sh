...
fetch_rule_api_ocn() {
    ...
    if ! echo "$raw_response" | jsonfilter -e '@.basicMapRules' >/dev/null 2>&1; then
        echo "ERROR: 無効なJSON応答を受信しました。" >&2
        return 1
    fi
    ...
    if ! echo "$raw_response" | jsonfilter -e '@.basicMapRules[0]' >/dev/null 2>&1; then
        return 1
    fi
    ...
    while true; do
        local ipv6_prefix
        ipv6_prefix=$(echo "$raw_response" | jsonfilter -e "@.basicMapRules[$i].ipv6Prefix" 2>/dev/null)
        
        if [ -z "$ipv6_prefix" ]; then
            break
        fi
        
        local api_prefix_rounded
        api_prefix_rounded=$(echo "$ipv6_prefix" | awk -F: '{printf("%s:%s:%s00::", $1, $2, substr($3, 1, 2))}')
        
        if [ "$api_prefix_rounded" = "$user_match_for_api_comparison" ]; then
            API_RESPONSE=$(echo "$raw_response" | jsonfilter -e "@.basicMapRules[$i]")
            
            if [ -n "$API_RESPONSE" ] && echo "$API_RESPONSE" | grep -q '{' && echo "$API_RESPONSE" | grep -q '}'; then
                return 0
            fi
        fi
...
display_mape() {
    ...
    printf "\033[1m注: ポート番号 1〜1023 は特権ポートです。詳細はISPに確認して下さい。\033[0m\n"
    printf "\n"
