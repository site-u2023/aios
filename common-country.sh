...
detect_and_set_location() {
    debug_log "DEBUG" "Running detect_and_set_location() function with improved detection sequence"
    ...
    # 2. dynamic-system-info.shから情報取得
    # スクリプトパスの確認
    ...
    # 3. システム情報から検出できなかった場合、process_location_infoを使用
    if [ -z "$system_country" ] || [ -z "$system_timezone" ]; then
        debug_log "DEBUG" "System detection failed, trying process_location_info()"
