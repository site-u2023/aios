...
#----- 1. ローカル IPv6 アドレスの取得 -----
LOCAL_IPV6=$(ip -6 addr show dev "$WAN_IFACE" scope global | awk '/inet6/ {print $2}' | cut -d'/' -f1 | head -n1)
if [ -z "$LOCAL_IPV6" ]; then
  echo "エラー: WAN インターフェース ($WAN_IFACE) のグローバルIPv6アドレスが取得できません。" >&2
  exit 1
fi
echo "取得したローカルIPv6アドレス: $LOCAL_IPV6"
...
RULE_IPV6_PREFIX=$(echo "$RULE_IPV6" | cut -d'/' -f1)
RULE_IPV6_PLEN=$(echo "$RULE_IPV6" | cut -d'/' -f2)
RULE_IPV4_NET=$(echo "$RULE_IPV4" | cut -d'/' -f1)
RULE_IPV4_PLEN=$(echo "$RULE_IPV4" | cut -d'/' -f2)
...
try:
    ...
    if local_ipv6 not in rule_ipv6_net:
        sys.exit("エラー: ローカルIPv6アドレス {} はルールのIPv6プレフィックス {} に含まれていません。".format(local_ipv6, rule_ipv6_net))
...
echo "計算された MAP-E クライアント用 IPv4 アドレス: $COMPUTED_IPV4"
