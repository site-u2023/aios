...
## **⚠️ ChatGPT への最優先ルール**  
本要件定義は、AIOSの仕様を厳密に定義するものとし、すべてのスクリプトはこの要件定義に基づいて設計・実装される。  
 **ChatGPTは以下のルールを厳守すること：**
1. **関数の上部に記載された `###` 内の要件を最優先とし、記述と異なる場合は必ずユーザーに確認すること。**
2. **本要件定義とスクリプトの `###` 内の要件が矛盾している場合、要件定義書を最優先とし、違いがある場合は修正を提案し、ユーザーに確認を求めること。**
3. **要件の不備・不明点がある場合、勝手に推測せずに必ずユーザーに確認を取ること。**
4. **スクリプトの変更・改善を提案する際は、本要件定義書を参照し、一貫性があることを保証すること。**
5. **新規関数の追加時は、必ず本要件定義書へ追加し、更新内容を明確にすること。**

🚨 **このルールは最優先で適用され、違反した場合は要件定義を修正するか、ユーザーに確認を取ること。**
```
#########################################################################　
# ＜具体的な要件＞
#########################################################################
