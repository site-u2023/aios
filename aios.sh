...
API_SETUP="e991edda6e25dec6d2bdcc320d395495408b42670babc8423f2ebd1dc3fa652ae2a6483e8d6f71c"
...
into_memory_message_definitions() {
    ...
    into_memory_message "en|MSG_ENTER_PASSWORD=New password{;}"
    into_memory_message "en|MSG_CONFIRM_PASSWORD=Retype new password{;}"
    ...
    into_memory_message "en|MSG_ENTER_HOSTNAME=New hostname{;}"
...
download_parallel() {
    ...
    if ! mkdir -p "$DL_DIR"; then
        ...
        DOWNLOAD_ELAPSED_SECONDS="$elapsed_seconds" 
    ...
    if ! grep -q . "$cmd_tmpfile"; then
        stop_spinner "$(get_message 'DOWNLOAD_PARALLEL_SUCCESS')" "success" # 変更点: s=... パラメータ削除
        end_time=$(date +%s)
        elapsed_seconds=$((end_time - start_time))
        DOWNLOAD_ELAPSED_SECONDS="$elapsed_seconds" 
        # printf "Download completed (no tasks) in %s seconds.\n" "$elapsed_seconds" # この行は不要になる想定
    ...
    if [ $overall_status -eq 0 ]; then
        ...
        elapsed_seconds=$((end_time - start_time))
        DOWNLOAD_ELAPSED_SECONDS="$elapsed_seconds" 
        success_message="$(get_message 'DOWNLOAD_PARALLEL_SUCCESS')" # 変更点: s=... パラメータ削除
...
print_information() {
    ...
    if [ ! -f "${CACHE_DIR}/message.ch" ]; then
        ...
        if [ -n "$DOWNLOAD_ELAPSED_SECONDS" ] && [ "$DOWNLOAD_ELAPSED_SECONDS" -gt 0 ] 2>/dev/null; then
            printf "%s\n" "$(color white "$(get_message "MSG_INFO_DEVICE_TIME" "s=$DOWNLOAD_ELAPSED_SECONDS")")"
        fi
    fi
...
download_files() {
    ...
    download "common-package-feed.sh" "chmod" "load"
