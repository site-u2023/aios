...
select_country() {
    ...
    if [ "$(echo "$tz_data" | wc -w)" -eq 2 ]; then
        ...
        case "$yn" in
            [Yy]*) ;;
            [Nn]*) echo "$(color yellow "Please try again.")"; continue ;;
            *) echo "$(color red "Invalid input. Please enter 'Y' or 'N'.")"; continue ;;
        esac
    else
        # **ゾーン選択**
    ...
    echo "$selected_zonename $selected_timezone" > "$language_cache"
