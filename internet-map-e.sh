...
mape_mold() {
    ...
    # --- BEGIN IPv6 HEXTET Parsing Enhancement (POSIX awk compliant, more robust) ---
    local hextet0_str hextet1_str hextet2_str hextet3_str # 16ビット区切りの16進文字列
    local awk_output

    # awkを使用して堅牢なIPv6アドレス解析と最初の4つのヘクステット抽出
    awk_output=$(echo "${ipv6_prefix}" | awk '
    BEGIN { FS=":"; OFS=":" } 
    {
        # 先頭/末尾の :: 特殊ケース処理
        if ($0 ~ /^::/) sub(/^::/, "0::")
        ...
        if ($0 ~ /::/) {
            ...
            for (i = 1; i <= zero_count; i++) {
                padding = padding "0" (i < zero_count ? ":" : "")
            }
            
            # アドレスを適切なパディングで再構築
            expanded = halves[1]
            if (halves[1] != "" && padding != "") expanded = expanded ":"
            expanded = expanded padding
            if (halves[2] != "" && padding != "") expanded = expanded ":"
            expanded = expanded halves[2]
        ...
        # 最初の4フィールドを抽出（存在しない場合は"0"をデフォルト値とする）
        h0 = (fields[1] != "" ? fields[1] : "0")
        h1 = (fields[2] != "" ? fields[2] : "0")
        h2 = (fields[3] != "" ? fields[3] : "0")
        h3 = (fields[4] != "" ? fields[4] : "0")
        
        # 最初の4つの16進文字列をスペース区切りで出力
        print h0 " " h1 " " h2 " " h3
    }')

    # awkの出力からスペース区切りの16進文字列をシェル変数に読み込み
    read -r hextet0_str hextet1_str hextet2_str hextet3_str <<EOF
$awk_output
EOF

    # awkが有効な出力を生成したか確認
    if [ -z "${hextet0_str}" ]; then
        printf "%s\n" "$(color red "Error: Failed to parse IPv6 prefix using awk.")"
        return 1
    fi

    # 16進文字列を10進数に変換
    local hextet0 hextet1 hextet2 hextet3
    hextet0=$(printf %d "0x${hextet0_str:-0}")
    hextet1=$(printf %d "0x${hextet1_str:-0}")
    hextet2=$(printf %d "0x${hextet2_str:-0}")
    hextet3=$(printf %d "0x${hextet3_str:-0}")
    ...
    # 16進形式のプレフィックス値取得
    local prefix31_hex prefix38_hex
    prefix31_hex=$(printf 0x%x "${prefix31}")
    prefix38_hex=$(printf 0x%x "${prefix38}")

    # IPv4アドレスとMAP-Eパラメータ計算
    local octet1 octet2 octet3 octet4 octet_values
    if [ -n "$(get_ruleprefix38_value "${prefix38_hex}")" ]; then
        # prefix38 ルールに基づくオクテット値取得
        octet_values="$(get_ruleprefix38_value "${prefix38_hex}")"
        debug_log "DEBUG" "Matched ruleprefix38: ${octet_values}"
        IFS=',' read -r octet1 octet2 octet3 <<EOF
${octet_values}
EOF
        local octet3_mask=$(( hextet2 & 768 ))    # 0x0300
        local octet3_bits=$(( octet3_mask >> 8 ))
        octet3=$(( octet3 | octet3_bits ))
        octet4=$(( hextet2 & 255 ))               # 0x00ff
        ...
        # prefix31 ルールに基づくオクテット値取得
        octet_values="$(get_ruleprefix31_value "${prefix31_hex}")"
        debug_log "DEBUG" "Matched ruleprefix31: ${octet_values}"
        IFS=',' read -r octet1 octet2 <<EOF
${octet_values}
EOF
        octet2=$(( octet2 | (hextet1 & 1) ))      # 0x0001
        local octet3_mask=$(( hextet2 & 65280 ))  # 0xff00
        octet3=$(( octet3_mask >> 8 ))
        octet4=$(( hextet2 & 255 ))               # 0x00ff
        ...
        # prefix38_20 ルールに基づくオクテット値取得
        octet_values="$(get_ruleprefix38_20_value "${prefix38_hex}")"
        debug_log "DEBUG" "Matched ruleprefix38_20: ${octet_values}"
        IFS=',' read -r octet1 octet2 octet3 <<EOF
${octet_values}
EOF
        local octet3_mask=$(( hextet2 & 960 ))    # 0x03c0
        local octet3_bits=$(( octet3_mask >> 6 ))
        octet3=$(( octet3 | octet3_bits ))
        
        local octet4_mask1=$(( hextet2 & 63 ))    # 0x003f
        local octet4_part1=$(( octet4_mask1 << 2 ))
        local octet4_mask2=$(( hextet3 & 49152 )) # 0xc000
        local octet4_part2=$(( octet4_mask2 >> 14 ))
        octet4=$(( octet4_part1 | octet4_part2 ))
    ...
    # PSID（Port Set ID）計算
    if [ "${PSIDLEN}" -eq 8 ]; then
        local psid_mask=$(( hextet3 & 65280 ))    # 0xff00
        PSID=$(( psid_mask >> 8 ))
        debug_log "DEBUG" "PSID calculation for PSIDLEN=8: ${PSID}"
    elif [ "${PSIDLEN}" -eq 6 ]; then
        local psid_mask=$(( hextet3 & 16128 ))    # 0x3f00
        PSID=$(( psid_mask >> 8 ))
        debug_log "DEBUG" "PSID calculation for PSIDLEN=6: ${PSID}"
    else
        PSID=0 # フォールバック
    ...
    debug_log "DEBUG" "Calculating port ranges: port_max_index=${port_max_index}, OFFSET=${OFFSET}, PSIDLEN=${PSIDLEN}, PSID=${PSID}"

    local port_index port_start port_end
    for port_index in $(seq 1 "${port_max_index}"); do
        local shift_bits=$(( 16 - OFFSET ))
        local port_base=$(( port_index << shift_bits ))
        local psid_shift=$(( 16 - OFFSET - PSIDLEN ))
        if [ "${psid_shift}" -lt 0 ]; then
            debug_log "DEBUG" "Invalid calculation: psid_shift is negative (${psid_shift}). Check OFFSET and PSIDLEN."
            psid_shift=0
        ...
        local psid_part=$(( PSID << psid_shift ))
        port_start=$(( port_base | psid_part ))
        local port_range_size=$(( 1 << psid_shift ))
        if [ "${port_range_size}" -le 0 ]; then
             debug_log "DEBUG" "Invalid calculation: port_range_size is not positive (${port_range_size})."
             port_range_size=1
        fi
        port_end=$(( port_start + port_range_size - 1 ))

        PORTS="${PORTS}${port_start}-${port_end}"

        if [ "${port_index}" -lt "${port_max_index}" ]; then
            if [ $(( port_index % 3 )) -eq 0 ]; then
                PORTS="${PORTS}\\n"
            else
                PORTS="${PORTS} "
            fi
    ...
    # CEアドレス文字列の生成
    local ce_str0 ce_str1 ce_str2 ce_str3 ce_str4 ce_str5 ce_str6 ce_str7
    ce_str0=$(printf %04x "${ce_hextet0}")
    ce_str1=$(printf %04x "${ce_hextet1}")
    ce_str2=$(printf %04x "${ce_hextet2}")
    ce_str3=$(printf %04x "${ce_hextet3}")
    ce_str4=$(printf %04x "${ce_hextet4}")
    ce_str5=$(printf %04x "${ce_hextet5}")
    ce_str6=$(printf %04x "${ce_hextet6}")
    ce_str7=$(printf %04x "${ce_hextet7}")
    CE="${ce_str0}:${ce_str1}:${ce_str2}:${ce_str3}:${ce_str4}:${ce_str5}:${ce_str6}:${ce_str7}"
    debug_log "DEBUG" "Generated CE address (CE): ${CE}"
    ...
    # IPv6プレフィックスの計算
    local ip6pfx_str0 ip6pfx_str1 ip6pfx_str2
    if [ "${IP6PREFIXLEN}" -eq 38 ]; then
        local hextet2_masked=$(( hextet2 & 64512 ))  # 0xfc00
        ip6pfx_str0=$(printf %x "${hextet0}")
        ip6pfx_str1=$(printf %x "${hextet1}")
        ip6pfx_str2=$(printf %x "${hextet2_masked}")
        IP6PFX="${ip6pfx_str0}:${ip6pfx_str1}:${ip6pfx_str2}"
    elif [ "${IP6PREFIXLEN}" -eq 31 ]; then
        local hextet1_masked=$(( hextet1 & 65534 ))  # 0xfffe
        ip6pfx_str0=$(printf %x "${hextet0}")
        ip6pfx_str1=$(printf %x "${hextet1_masked}")
        IP6PFX="${ip6pfx_str0}:${ip6pfx_str1}"
    else
        IP6PFX="" # フォールバック
        debug_log "WARNING" "Could not determine IP6PFX for IP6PREFIXLEN=${IP6PREFIXLEN}"
    fi
    ...
    if [ -n "$PORTS" ]; then
        ...
        for A in $(seq 1 "$port_max_index"); do
            ...
            if [ "$items_in_line" -eq 0 ]; then
                line_buffer="${port_start}-${port_end}"
            else
                line_buffer="${line_buffer} ${port_start}-${port_end}"
            fi
