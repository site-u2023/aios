...
get_country_code() {
    ...
    if [ "$network_type" = "v4" ] || [ "$network_type" = "v4v6" ]; then
        ...
        if [ -n "$ip_v4" ]; then
            ...
            update_spinner "$(color blue "API接続中: $API_WORLDTIME (IPv4)")"
    ...
    if { [ -z "$SELECT_ZONE" ] || ! echo "$SELECT_ZONE" | grep -q '"timezone"' || ! echo "$SELECT_ZONE" | grep -q '"abbreviation"' || ! echo "$SELECT_ZONE" | grep -q '"utc_offset"'; } && { [ "$network_type" = "v6" ] || [ "$network_type" = "v4v6" ]; }; then
        ...
        # IPv6アドレスの取得を試行
        if [ -z "$ip_v6" ]; then
            update_spinner "$(color blue "API接続中: $API_IPV6 (IPv6)")"
        ...
        if [ -n "$ip_v6" ]; then
            update_spinner "$(color blue "API接続中: $API_WORLDTIME (IPv6)")"
    ...
    if [ -n "$SELECT_ZONE" ]; then
        ...
        if [ -n "$worldtime_ip" ]; then
            update_spinner "$(color blue "API接続中: $API_IPAPI ($select_ip_ver)")"
    ...
    if [ -z "$SELECT_COUNTRY" ]; then
        ...
        if [ -n "$fallback_ip" ]; then
            update_spinner "$(color blue "API接続中: $API_IPAPI ($fallback_network)")"
            ...
            if [ -f "$tmp_file" ] && [ -s "$tmp_file" ]; then
                ...
                if [ -n "$fallback_data" ]; then
                    ...
                    if [ -n "$SELECT_COUNTRY" ]; then
                        ...
                        update_spinner "$(color blue "API接続中: $API_IPINFO ($fallback_network)")"
    ...
    if [ -n "$SELECT_ZONENAME" ] && [ -n "$SELECT_TIMEZONE" ] && [ -n "$SELECT_COUNTRY" ]; then
        # 成功時
        stop_spinner "$(color green "情報取得完了")" "success"
        debug_log "DEBUG: Successfully retrieved all required information"
        return 0
    else
        # 失敗時
        stop_spinner "$(color red "取得失敗")" "error"
