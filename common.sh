...
COMMON_VERSION="2025.02.08-00024"
...
select_country() {
    ...
    while true; do
        if [ "$search_mode" = "initial" ]; then
            ...
            if echo "$choice" | grep -qE '^[0-9]+$'; then
                ...
                echo "$(color red "Invalid input. Please enter a number from the list.")"
            fi
        fi
    done
    ...
    if echo "$tz_data" | grep -q ","; then
        ...
        while true; do
            ...
            read tz_choice
            if echo "$tz_choice" | grep -qE '^[0-9]+$' && [ "$tz_choice" -ge 1 ] && [ "$tz_choice" -le $# ]; then
                selected_zone_name=$(eval echo \$$tz_choice)
                selected_timezone=$(eval echo \$$tz_choice)
            else
