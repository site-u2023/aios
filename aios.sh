...
get_message() {
    ...
    if [ -f "${CACHE_DIR}/message.ch" ]; then
        # OPTIMIZATION: Use read instead of cat (reduces process creation)
        read -r lang < "${CACHE_DIR}/message.ch"
    fi

    # 1. Get message from DB file cache
    local db_file="$(check_message_cache "$lang")" # Assumes check_message_cache exists
    if [ -n "$db_file" ] && [ -f "$db_file" ]; then
        # OPTIMIZATION: Retrieve message with fewer processes
        local raw_line
        raw_line=$(grep "^${lang}|${key}=" "$db_file" 2>/dev/null)
        if [ -n "$raw_line" ]; then
            message="${raw_line#*=}"
        fi
        ...
        if [ -z "$message" ] && [ "$lang" != "$DEFAULT_LANGUAGE" ]; then
            ...
            if [ -n "$default_db_file" ] && [ -f "$default_db_file" ]; then
                raw_line=$(grep "^${DEFAULT_LANGUAGE}|${key}=" "$default_db_file" 2>/dev/null)
                if [ -n "$raw_line" ]; then
                    message="${raw_line#*=}"
                fi
            fi
    ...
    if [ -z "$message" ]; then
        ...
        # Retrieve message from memory cache with optimized processing
        if [ -n "$MSG_MEMORY" ]; then
            local raw_line
            raw_line=$(echo "$MSG_MEMORY" | grep "^${lang}|${key}=" 2>/dev/null)
            if [ -n "$raw_line" ]; then
                message="${raw_line#*=}"
            fi
            ...
            if [ -z "$message" ] && [ "$lang" != "$DEFAULT_LANGUAGE" ]; then
                raw_line=$(echo "$MSG_MEMORY" | grep "^${DEFAULT_LANGUAGE}|${key}=" 2>/dev/null)
                if [ -n "$raw_line" ]; then
                    message="${raw_line#*=}"
                fi
            fi
    ...
    # --- OPTIMIZATION: Replace echo+sed with parameter expansion for brace normalization ---
    ...
    # (The awk script remains unchanged for compatibility)
    awk_script='
        BEGIN { FS="=" }
    ...
    '
    # OPTIMIZATION: Improved parameter handling with fewer process creations
    if [ $# -gt 0 ]; then
        message=$( \
            ( \
                printf "%s\n" "$message"; \
                ...
                for param in "$@"; do \
                    # Use parameter expansion instead of echo+cut
                    param_name="${param%%=*}";
                    if [ "$param" = "$param_name" ]; then
                        # No equals sign in the parameter
                        param_value=""
                    else
                        param_value="${param#*=}";
                    fi
