...
# IPv6アドレスの取得
ocn_get_ipv6_address() {
    local ipv6_addr=""
    local net_if6=""
    
    # OpenWrtのネットワーク関数を使用
    if ocn_load_network_libs; then
...
ocn_is_ipv6_ocn() {
    local ipv6="$1"
    local ipv6_prefix_pattern="2001:0*260"
    
    if echo "$ipv6" | grep -q "^$ipv6_prefix_pattern"; then
        debug_log "DEBUG" "IPv6 address matches OCN pattern"
        return 0
    else
        debug_log "DEBUG" "IPv6 address does not match OCN pattern"
        return 1
    fi
}

# IPv6プレフィックスを取得（OCN向け）
ocn_get_ipv6_prefix() {
    local ipv6_addr="$1"
    local ipv6_prefix_pattern="2001:0*260"
    local ipv6_prefix_length="28"
    
    debug_log "DEBUG" "Extracting OCN IPv6 prefix from: $ipv6_addr"
    ...
    # OCNのIPv6アドレスかチェック
    if ! ocn_is_ipv6_ocn "$ipv6_addr"; then
        printf "%s\n" "$(color red "このIPv6アドレスはOCNのものではありません (2001:0260で始まりません)")" >&2
        return 1
    fi
    
    # プレフィックス部分の取得
    local prefix
    prefix=$(echo "$ipv6_addr" | awk -F: '{print $1":"$2}')
    
    if [ -n "$prefix" ]; then
        local full_prefix="${prefix}:0:0:0:0:0:0/${ipv6_prefix_length}"
        debug_log "DEBUG" "Extracted OCN IPv6 prefix: $full_prefix"
        echo "$full_prefix"
        return 0
    fi
...
# OCN MAP-E設定を適用
ocn_apply_mape_config() {
    local ipv6_prefix="$1"
    local major_version="$2"
    
    # OCN MAP-E固定パラメータ（ローカル変数）
    ...
    # 設定のバックアップ作成
    cp /etc/config/network /etc/config/network.ocn.bak 2>/dev/null
    cp /etc/config/firewall /etc/config/firewall.ocn.bak 2>/dev/null
    cp /etc/config/dhcp /etc/config/dhcp.ocn.bak 2>/dev/null
...
# メイン処理
Internet_ocn_main() {
    printf "%s\n" "$(color blue "OCN MAP-E設定スクリプト v${SCRIPT_VERSION}")"
    printf "%s\n" "$(color blue "========================================")"
    
    # IPv6アドレスの取得
    printf "%s\n" "$(color green "IPv6アドレスを取得中...")"
    local ipv6_addr
    ipv6_addr=$(ocn_get_ipv6_address)
    ...
    # OCNのIPv6アドレスか確認
    if ! ocn_is_ipv6_ocn "$ipv6_addr"; then
        printf "%s\n" "$(color red "エラー: このIPv6アドレスはOCN回線のものではないようです")"
        exit 1
    fi
    printf "%s\n" "$(color green "OCN回線のIPv6アドレスを確認しました")"
    
    # IPv6プレフィックスの取得
    printf "%s\n" "$(color green "IPv6プレフィックスを取得中...")"
    local ipv6_prefix
    ipv6_prefix=$(ocn_get_ipv6_prefix "$ipv6_addr")
    ...
    # OpenWrtバージョンの取得
    local major_version
    major_version=$(ocn_get_openwrt_version)
    printf "%s: %s\n" "$(color cyan "OpenWrtバージョン")" "$major_version"
    
    # MAP-E設定の適用
    ocn_apply_mape_config "$ipv6_prefix" "$major_version"
